...
class TabPicker extends Component {
  ...
  componentWillMount () {
    this.props.getTopicListAsync().then(res => {
      this.setState({
        topicList: this.sortList(this.props.topicList)
      })
    })
  }
  ...
  onDragEnd = result => {
    ...
    this.setState({ topicList: items }, () => {
      this.props.resetTopicListAsync(this.state.topicList)
    })
  }
...
const mapDispatch = ({ activity: { resetTopicListAsync, getTopicListAsync } }) => ({
  resetTopicListAsync: topicList => resetTopicListAsync({ topicList }),
